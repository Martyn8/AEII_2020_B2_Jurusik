...
function czy_spoznienie_na_wyjsciu_z_basenu($wyjscie, $wejscie, $ile_godzin_na_bilecie){ //Funkcja do liczenia czy ycześtnik się spóźnił czy nie.
    ...
    if($secs<$ile_godzin_na_bilecie*-3600){
		echo "spoznienie o ".$ile_spoznienia." minuty";
		// echo -($ile_godzin_na_bilecie - $secs*3600) % 60;
		return $ile_spoznienia;
    }
    else{
		echo "brak spoznienia";
		return 0;
    }
}
...
function walidacja_danych_platnosci($imie,$nazwisko,$email_1,$email_2,$telefon,$methods,$regulamin){
	...
	{
		$all_ok=false;
		$_SESSION['err_email_1']='<br /><span style="color:red"><b>Nieprawidłowy adres e-mail.</b></span>';
		unset($_SESSION['ch_email_1']);
	}
	if($email_1!=$email_2)
	{
		$all_ok=false;
		$_SESSION['err_email_2']='<br /><span style="color:red"><b>Adres e-mail się nie zgadza.</b></span>';
		unset($_SESSION['ch_email_2']);
	}
	if(!is_numeric($telefon)|| strlen($telefon)!=9)
	{
		$all_ok=false;
		$_SESSION['err_telefon']='<br /><span style="color:red"><b>Nieprawidłowy numer telefonu.</b></span>';
		unset($_SESSION['ch_telefon']);
	}
	if(!isset($methods))
	{
		$all_ok=false;
		$_SESSION['err_methods']='<br /><span style="color:red"><b>Nie wybrano metody płatności.</b></span><br />';
		unset($_SESSION['ch_methods']);
	}
	if(!isset($regulamin))
	{
		$all_ok=false;
		$_SESSION['err_regulamin']='<span style="color:red"><b>Nie zaakceptowano regulaminu.</b></span><br />';
...
function walidacja_danych_karty($imie,$nr_karty,$trzy_cyfry,$data_waznosci){
	...
	{
		$all_ok=false;
		$_SESSION['err_pl_master']='<br /><span style="color:red"><b>Nieprawidłowy numer karty.</b></span>';
		unset($_SESSION['ch_pl_master']);
	}
	if(!is_numeric($trzy_cyfry)|| strlen($trzy_cyfry)!=3)
	{
		$all_ok=false;
		$_SESSION['err_pl_trzy_cyfry']='<br /><span style="color:red"><b>Nieprawidłowy kod bezpieczeństwa.</b></span>';
	...
	{
		$all_ok=false;
		$_SESSION['err_pl_data']='<br /><span style="color:red"><b>Nieprawidłowa data.</b></span>';

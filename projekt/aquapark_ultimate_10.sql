-- phpMyAdmin SQL Dump
-- version 5.0.4
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1
-- Czas generowania: 06 Gru 2020, 14:48
-- Wersja serwera: 10.4.17-MariaDB
-- Wersja PHP: 7.4.13

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Baza danych: `aquapark`
--

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `atrakcja`
--

CREATE TABLE `atrakcja` (
  `id_atrakcji` int(11) NOT NULL,
  `typ_atrakcji` char(20) COLLATE utf8_polish_ci NOT NULL,
  `rodzaj_atrakcji` char(20) COLLATE utf8_polish_ci NOT NULL,
  `zegarek_id_zegarka` int(11) DEFAULT NULL,
  `cennik_id_cennika` int(11) NOT NULL,
  `lista_atrakcji_nazwa` char(30) COLLATE utf8_polish_ci NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `bilet`
--

CREATE TABLE `bilet` (
  `id_biletu` int(11) NOT NULL,
  `typ_biletu` char(20) COLLATE utf8_polish_ci NOT NULL,
  `rodzaj_biletu` char(20) COLLATE utf8_polish_ci NOT NULL,
  `czas` int(11) NOT NULL,
  `klient_id_klienta` int(11) NOT NULL,
  `platnosc_id_platnosci` int(11) NOT NULL,
  `cennik_id_cennika` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `cennik`
--

CREATE TABLE `cennik` (
  `id_cennika` int(11) NOT NULL,
  `typ` char(20) COLLATE utf8_polish_ci NOT NULL,
  `rodzaj` char(20) COLLATE utf8_polish_ci NOT NULL,
  `czas` varchar(11) COLLATE utf8_polish_ci DEFAULT NULL,
  `cena` float DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

--
-- Zrzut danych tabeli `cennik`
--

INSERT INTO `cennik` (`id_cennika`, `typ`, `rodzaj`, `czas`, `cena`) VALUES
(1, 'Bilet', 'Ulgowy', '1', 15),
(2, 'Bilet', 'Ulgowy', '3', 30),
(3, 'Bilet', 'Ulgowy', '5', 35),
(4, 'Bilet', 'Ulgowy', '12', 50),
(5, 'Bilet', 'Normalny', '1', 20),
(6, 'Bilet', 'Normalny', '3', 35),
(7, 'Bilet', 'Normalny', '5', 50),
(8, 'Bilet', 'Normalny', '12', 70),
(9, 'Karnet', 'Ulgowy', 'Tygodniowy', 250),
(10, 'Karnet', 'Ulgowy', 'Miesięczny', 350),
(11, 'Karnet', 'Normalny', 'Tygodniowy', 300),
(12, 'Karnet', 'Normalny', 'Miesięczny', 450),
(13, 'Atrakcja', 'Sauna fińska', NULL, 20),
(14, 'Atrakcja', 'Sauna ziołowa', NULL, 25),
(15, 'Atrakcja', 'Sauna parowa', NULL, 15),
(16, 'Atrakcja', 'Sauna na podczerwień', NULL, 15),
(17, 'Atrakcja', 'Biosauna', NULL, 20),
(18, 'Kara', 'Przedłużenie pobytu', '1', 0.5),
(19, 'Kara', 'Zgubienie opaski', NULL, 300);

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `doplata`
--

CREATE TABLE `doplata` (
  `id_doplaty` int(11) NOT NULL,
  `kwota_doplaty` float NOT NULL,
  `rodzaj_platnosci` char(30) COLLATE utf8_polish_ci NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `karnet`
--

CREATE TABLE `karnet` (
  `id_karnetu` int(11) NOT NULL,
  `typ_karnetu` char(20) COLLATE utf8_polish_ci NOT NULL,
  `rodzaj_karnetu` char(20) COLLATE utf8_polish_ci NOT NULL,
  `czas` varchar(11) COLLATE utf8_polish_ci NOT NULL,
  `data_aktywacji` date DEFAULT NULL,
  `data_waznosci` date DEFAULT NULL,
  `klient_id_klienta` int(11) NOT NULL,
  `platnosc_id_platnosci` int(11) NOT NULL,
  `cennik_id_cennika` int(11) NOT NULL,
  `kod_weryfikacyjny_karnetu` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `klient`
--

CREATE TABLE `klient` (
  `id_klienta` int(11) NOT NULL,
  `imie` char(20) COLLATE utf8_polish_ci DEFAULT NULL,
  `nazwisko` char(30) COLLATE utf8_polish_ci DEFAULT NULL,
  `mail` char(50) COLLATE utf8_polish_ci DEFAULT NULL,
  `telefon` int(11) DEFAULT NULL,
  `kod_weryfikacyjny` varchar(6) COLLATE utf8_polish_ci DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `konta_w_systemie`
--

CREATE TABLE `konta_w_systemie` (
  `login` char(30) COLLATE utf8_polish_ci NOT NULL,
  `haslo` char(20) COLLATE utf8_polish_ci NOT NULL,
  `typ_konta` char(10) COLLATE utf8_polish_ci NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

--
-- Zrzut danych tabeli `konta_w_systemie`
--

INSERT INTO `konta_w_systemie` (`login`, `haslo`, `typ_konta`) VALUES
('admin', '1234', 'zarządca'),
('prac', '1234', 'pracownik');

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `lista_atrakcji`
--

CREATE TABLE `lista_atrakcji` (
  `nazwa` char(30) COLLATE utf8_polish_ci NOT NULL,
  `strefa` char(30) COLLATE utf8_polish_ci DEFAULT NULL,
  `opis` text COLLATE utf8_polish_ci DEFAULT NULL,
  `zdjecie` blob DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

--
-- Zrzut danych tabeli `lista_atrakcji`
--

INSERT INTO `lista_atrakcji` (`nazwa`, `strefa`, `opis`, `zdjecie`) VALUES
('Biosauna', NULL, NULL, NULL),
('Sauna fińska', NULL, NULL, NULL),
('Sauna na podczerwień', NULL, NULL, NULL),
('Sauna parowa', NULL, NULL, NULL),
('Sauna ziołowa', NULL, NULL, NULL);

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `platnosc`
--

CREATE TABLE `platnosc` (
  `id_platnosci` int(11) NOT NULL,
  `kwota` float NOT NULL,
  `rodzaj_platnosci` char(30) COLLATE utf8_polish_ci NOT NULL,
  `klient_id_klienta` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

-- --------------------------------------------------------

--
-- Struktura tabeli dla tabeli `zegarek`
--

CREATE TABLE `zegarek` (
  `id_zegarka` int(11) NOT NULL,
  `klient_id_klienta` int(11) DEFAULT NULL,
  `bilet_id_biletu` int(11) DEFAULT NULL,
  `karnet_id_karnetu` int(11) DEFAULT NULL,
  `czas_wejscia` datetime DEFAULT NULL,
  `czas_wyjscia` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

--
-- Zrzut danych tabeli `zegarek`
--

INSERT INTO `zegarek` (`id_zegarka`, `klient_id_klienta`, `bilet_id_biletu`, `karnet_id_karnetu`, `czas_wejscia`, `czas_wyjscia`) VALUES
(1, NULL, NULL, NULL, NULL, NULL),
(2, NULL, NULL, NULL, NULL, NULL),
(3, NULL, NULL, NULL, NULL, NULL),
(4, NULL, NULL, NULL, NULL, NULL),
(5, NULL, NULL, NULL, NULL, NULL),
(6, NULL, NULL, NULL, NULL, NULL),
(7, NULL, NULL, NULL, NULL, NULL),
(8, NULL, NULL, NULL, NULL, NULL),
(9, NULL, NULL, NULL, NULL, NULL),
(10, NULL, NULL, NULL, NULL, NULL),
(11, NULL, NULL, NULL, NULL, NULL),
(12, NULL, NULL, NULL, NULL, NULL),
(13, NULL, NULL, NULL, NULL, NULL),
(14, NULL, NULL, NULL, NULL, NULL),
(15, NULL, NULL, NULL, NULL, NULL),
(16, NULL, NULL, NULL, NULL, NULL),
(17, NULL, NULL, NULL, NULL, NULL),
(18, NULL, NULL, NULL, NULL, NULL),
(19, NULL, NULL, NULL, NULL, NULL),
(20, NULL, NULL, NULL, NULL, NULL),
(21, NULL, NULL, NULL, NULL, NULL),
(22, NULL, NULL, NULL, NULL, NULL),
(23, NULL, NULL, NULL, NULL, NULL),
(24, NULL, NULL, NULL, NULL, NULL),
(25, NULL, NULL, NULL, NULL, NULL),
(26, NULL, NULL, NULL, NULL, NULL),
(27, NULL, NULL, NULL, NULL, NULL),
(28, NULL, NULL, NULL, NULL, NULL),
(29, NULL, NULL, NULL, NULL, NULL),
(30, NULL, NULL, NULL, NULL, NULL),
(31, NULL, NULL, NULL, NULL, NULL),
(32, NULL, NULL, NULL, NULL, NULL),
(33, NULL, NULL, NULL, NULL, NULL),
(34, NULL, NULL, NULL, NULL, NULL),
(35, NULL, NULL, NULL, NULL, NULL),
(36, NULL, NULL, NULL, NULL, NULL),
(37, NULL, NULL, NULL, NULL, NULL),
(38, NULL, NULL, NULL, NULL, NULL),
(39, NULL, NULL, NULL, NULL, NULL),
(40, NULL, NULL, NULL, NULL, NULL),
(41, NULL, NULL, NULL, NULL, NULL),
(42, NULL, NULL, NULL, NULL, NULL),
(43, NULL, NULL, NULL, NULL, NULL),
(44, NULL, NULL, NULL, NULL, NULL),
(45, NULL, NULL, NULL, NULL, NULL),
(46, NULL, NULL, NULL, NULL, NULL),
(47, NULL, NULL, NULL, NULL, NULL),
(48, NULL, NULL, NULL, NULL, NULL),
(49, NULL, NULL, NULL, NULL, NULL),
(50, NULL, NULL, NULL, NULL, NULL),
(51, NULL, NULL, NULL, NULL, NULL),
(52, NULL, NULL, NULL, NULL, NULL),
(53, NULL, NULL, NULL, NULL, NULL),
(54, NULL, NULL, NULL, NULL, NULL),
(55, NULL, NULL, NULL, NULL, NULL),
(56, NULL, NULL, NULL, NULL, NULL),
(57, NULL, NULL, NULL, NULL, NULL),
(58, NULL, NULL, NULL, NULL, NULL),
(59, NULL, NULL, NULL, NULL, NULL),
(60, NULL, NULL, NULL, NULL, NULL),
(61, NULL, NULL, NULL, NULL, NULL),
(62, NULL, NULL, NULL, NULL, NULL),
(63, NULL, NULL, NULL, NULL, NULL),
(64, NULL, NULL, NULL, NULL, NULL),
(65, NULL, NULL, NULL, NULL, NULL),
(66, NULL, NULL, NULL, NULL, NULL),
(67, NULL, NULL, NULL, NULL, NULL),
(68, NULL, NULL, NULL, NULL, NULL),
(69, NULL, NULL, NULL, NULL, NULL),
(70, NULL, NULL, NULL, NULL, NULL),
(71, NULL, NULL, NULL, NULL, NULL),
(72, NULL, NULL, NULL, NULL, NULL),
(73, NULL, NULL, NULL, NULL, NULL),
(74, NULL, NULL, NULL, NULL, NULL),
(75, NULL, NULL, NULL, NULL, NULL),
(76, NULL, NULL, NULL, NULL, NULL),
(77, NULL, NULL, NULL, NULL, NULL),
(78, NULL, NULL, NULL, NULL, NULL),
(79, NULL, NULL, NULL, NULL, NULL),
(80, NULL, NULL, NULL, NULL, NULL),
(81, NULL, NULL, NULL, NULL, NULL),
(82, NULL, NULL, NULL, NULL, NULL),
(83, NULL, NULL, NULL, NULL, NULL),
(84, NULL, NULL, NULL, NULL, NULL),
(85, NULL, NULL, NULL, NULL, NULL),
(86, NULL, NULL, NULL, NULL, NULL),
(87, NULL, NULL, NULL, NULL, NULL),
(88, NULL, NULL, NULL, NULL, NULL),
(89, NULL, NULL, NULL, NULL, NULL),
(90, NULL, NULL, NULL, NULL, NULL),
(91, NULL, NULL, NULL, NULL, NULL),
(92, NULL, NULL, NULL, NULL, NULL),
(93, NULL, NULL, NULL, NULL, NULL),
(94, NULL, NULL, NULL, NULL, NULL),
(95, NULL, NULL, NULL, NULL, NULL),
(96, NULL, NULL, NULL, NULL, NULL),
(97, NULL, NULL, NULL, NULL, NULL),
(98, NULL, NULL, NULL, NULL, NULL),
(99, NULL, NULL, NULL, NULL, NULL),
(100, NULL, NULL, NULL, NULL, NULL),
(101, NULL, NULL, NULL, NULL, NULL),
(102, NULL, NULL, NULL, NULL, NULL),
(103, NULL, NULL, NULL, NULL, NULL),
(104, NULL, NULL, NULL, NULL, NULL),
(105, NULL, NULL, NULL, NULL, NULL),
(106, NULL, NULL, NULL, NULL, NULL),
(107, NULL, NULL, NULL, NULL, NULL),
(108, NULL, NULL, NULL, NULL, NULL),
(109, NULL, NULL, NULL, NULL, NULL),
(110, NULL, NULL, NULL, NULL, NULL),
(111, NULL, NULL, NULL, NULL, NULL),
(112, NULL, NULL, NULL, NULL, NULL),
(113, NULL, NULL, NULL, NULL, NULL),
(114, NULL, NULL, NULL, NULL, NULL),
(115, NULL, NULL, NULL, NULL, NULL),
(116, NULL, NULL, NULL, NULL, NULL),
(117, NULL, NULL, NULL, NULL, NULL),
(118, NULL, NULL, NULL, NULL, NULL),
(119, NULL, NULL, NULL, NULL, NULL),
(120, NULL, NULL, NULL, NULL, NULL),
(121, NULL, NULL, NULL, NULL, NULL),
(122, NULL, NULL, NULL, NULL, NULL),
(123, NULL, NULL, NULL, NULL, NULL),
(124, NULL, NULL, NULL, NULL, NULL),
(125, NULL, NULL, NULL, NULL, NULL),
(126, NULL, NULL, NULL, NULL, NULL),
(127, NULL, NULL, NULL, NULL, NULL),
(128, NULL, NULL, NULL, NULL, NULL),
(129, NULL, NULL, NULL, NULL, NULL),
(130, NULL, NULL, NULL, NULL, NULL),
(131, NULL, NULL, NULL, NULL, NULL),
(132, NULL, NULL, NULL, NULL, NULL),
(133, NULL, NULL, NULL, NULL, NULL),
(134, NULL, NULL, NULL, NULL, NULL),
(135, NULL, NULL, NULL, NULL, NULL),
(136, NULL, NULL, NULL, NULL, NULL),
(137, NULL, NULL, NULL, NULL, NULL),
(138, NULL, NULL, NULL, NULL, NULL),
(139, NULL, NULL, NULL, NULL, NULL),
(140, NULL, NULL, NULL, NULL, NULL),
(141, NULL, NULL, NULL, NULL, NULL),
(142, NULL, NULL, NULL, NULL, NULL),
(143, NULL, NULL, NULL, NULL, NULL),
(144, NULL, NULL, NULL, NULL, NULL),
(145, NULL, NULL, NULL, NULL, NULL),
(146, NULL, NULL, NULL, NULL, NULL),
(147, NULL, NULL, NULL, NULL, NULL),
(148, NULL, NULL, NULL, NULL, NULL),
(149, NULL, NULL, NULL, NULL, NULL),
(150, NULL, NULL, NULL, NULL, NULL),
(151, NULL, NULL, NULL, NULL, NULL),
(152, NULL, NULL, NULL, NULL, NULL),
(153, NULL, NULL, NULL, NULL, NULL),
(154, NULL, NULL, NULL, NULL, NULL),
(155, NULL, NULL, NULL, NULL, NULL),
(156, NULL, NULL, NULL, NULL, NULL),
(157, NULL, NULL, NULL, NULL, NULL),
(158, NULL, NULL, NULL, NULL, NULL),
(159, NULL, NULL, NULL, NULL, NULL),
(160, NULL, NULL, NULL, NULL, NULL),
(161, NULL, NULL, NULL, NULL, NULL),
(162, NULL, NULL, NULL, NULL, NULL),
(163, NULL, NULL, NULL, NULL, NULL),
(164, NULL, NULL, NULL, NULL, NULL),
(165, NULL, NULL, NULL, NULL, NULL),
(166, NULL, NULL, NULL, NULL, NULL),
(167, NULL, NULL, NULL, NULL, NULL),
(168, NULL, NULL, NULL, NULL, NULL),
(169, NULL, NULL, NULL, NULL, NULL),
(170, NULL, NULL, NULL, NULL, NULL),
(171, NULL, NULL, NULL, NULL, NULL),
(172, NULL, NULL, NULL, NULL, NULL),
(173, NULL, NULL, NULL, NULL, NULL),
(174, NULL, NULL, NULL, NULL, NULL),
(175, NULL, NULL, NULL, NULL, NULL),
(176, NULL, NULL, NULL, NULL, NULL),
(177, NULL, NULL, NULL, NULL, NULL),
(178, NULL, NULL, NULL, NULL, NULL),
(179, NULL, NULL, NULL, NULL, NULL),
(180, NULL, NULL, NULL, NULL, NULL),
(181, NULL, NULL, NULL, NULL, NULL),
(182, NULL, NULL, NULL, NULL, NULL),
(183, NULL, NULL, NULL, NULL, NULL),
(184, NULL, NULL, NULL, NULL, NULL),
(185, NULL, NULL, NULL, NULL, NULL),
(186, NULL, NULL, NULL, NULL, NULL),
(187, NULL, NULL, NULL, NULL, NULL),
(188, NULL, NULL, NULL, NULL, NULL),
(189, NULL, NULL, NULL, NULL, NULL),
(190, NULL, NULL, NULL, NULL, NULL),
(191, NULL, NULL, NULL, NULL, NULL),
(192, NULL, NULL, NULL, NULL, NULL),
(193, NULL, NULL, NULL, NULL, NULL),
(194, NULL, NULL, NULL, NULL, NULL),
(195, NULL, NULL, NULL, NULL, NULL),
(196, NULL, NULL, NULL, NULL, NULL),
(197, NULL, NULL, NULL, NULL, NULL),
(198, NULL, NULL, NULL, NULL, NULL),
(199, NULL, NULL, NULL, NULL, NULL),
(200, NULL, NULL, NULL, NULL, NULL),
(201, NULL, NULL, NULL, NULL, NULL),
(202, NULL, NULL, NULL, NULL, NULL),
(203, NULL, NULL, NULL, NULL, NULL),
(204, NULL, NULL, NULL, NULL, NULL),
(205, NULL, NULL, NULL, NULL, NULL),
(206, NULL, NULL, NULL, NULL, NULL),
(207, NULL, NULL, NULL, NULL, NULL),
(208, NULL, NULL, NULL, NULL, NULL),
(209, NULL, NULL, NULL, NULL, NULL),
(210, NULL, NULL, NULL, NULL, NULL),
(211, NULL, NULL, NULL, NULL, NULL),
(212, NULL, NULL, NULL, NULL, NULL),
(213, NULL, NULL, NULL, NULL, NULL),
(214, NULL, NULL, NULL, NULL, NULL),
(215, NULL, NULL, NULL, NULL, NULL),
(216, NULL, NULL, NULL, NULL, NULL),
(217, NULL, NULL, NULL, NULL, NULL),
(218, NULL, NULL, NULL, NULL, NULL),
(219, NULL, NULL, NULL, NULL, NULL),
(220, NULL, NULL, NULL, NULL, NULL),
(221, NULL, NULL, NULL, NULL, NULL),
(222, NULL, NULL, NULL, NULL, NULL),
(223, NULL, NULL, NULL, NULL, NULL),
(224, NULL, NULL, NULL, NULL, NULL),
(225, NULL, NULL, NULL, NULL, NULL),
(226, NULL, NULL, NULL, NULL, NULL),
(227, NULL, NULL, NULL, NULL, NULL),
(228, NULL, NULL, NULL, NULL, NULL),
(229, NULL, NULL, NULL, NULL, NULL),
(230, NULL, NULL, NULL, NULL, NULL),
(231, NULL, NULL, NULL, NULL, NULL),
(232, NULL, NULL, NULL, NULL, NULL),
(233, NULL, NULL, NULL, NULL, NULL),
(234, NULL, NULL, NULL, NULL, NULL),
(235, NULL, NULL, NULL, NULL, NULL),
(236, NULL, NULL, NULL, NULL, NULL),
(237, NULL, NULL, NULL, NULL, NULL),
(238, NULL, NULL, NULL, NULL, NULL),
(239, NULL, NULL, NULL, NULL, NULL),
(240, NULL, NULL, NULL, NULL, NULL),
(241, NULL, NULL, NULL, NULL, NULL),
(242, NULL, NULL, NULL, NULL, NULL),
(243, NULL, NULL, NULL, NULL, NULL),
(244, NULL, NULL, NULL, NULL, NULL),
(245, NULL, NULL, NULL, NULL, NULL),
(246, NULL, NULL, NULL, NULL, NULL),
(247, NULL, NULL, NULL, NULL, NULL),
(248, NULL, NULL, NULL, NULL, NULL),
(249, NULL, NULL, NULL, NULL, NULL),
(250, NULL, NULL, NULL, NULL, NULL),
(251, NULL, NULL, NULL, NULL, NULL),
(252, NULL, NULL, NULL, NULL, NULL),
(253, NULL, NULL, NULL, NULL, NULL),
(254, NULL, NULL, NULL, NULL, NULL),
(255, NULL, NULL, NULL, NULL, NULL),
(256, NULL, NULL, NULL, NULL, NULL),
(257, NULL, NULL, NULL, NULL, NULL),
(258, NULL, NULL, NULL, NULL, NULL),
(259, NULL, NULL, NULL, NULL, NULL),
(260, NULL, NULL, NULL, NULL, NULL),
(261, NULL, NULL, NULL, NULL, NULL),
(262, NULL, NULL, NULL, NULL, NULL),
(263, NULL, NULL, NULL, NULL, NULL),
(264, NULL, NULL, NULL, NULL, NULL),
(265, NULL, NULL, NULL, NULL, NULL),
(266, NULL, NULL, NULL, NULL, NULL),
(267, NULL, NULL, NULL, NULL, NULL),
(268, NULL, NULL, NULL, NULL, NULL),
(269, NULL, NULL, NULL, NULL, NULL),
(270, NULL, NULL, NULL, NULL, NULL),
(271, NULL, NULL, NULL, NULL, NULL),
(272, NULL, NULL, NULL, NULL, NULL),
(273, NULL, NULL, NULL, NULL, NULL),
(274, NULL, NULL, NULL, NULL, NULL),
(275, NULL, NULL, NULL, NULL, NULL),
(276, NULL, NULL, NULL, NULL, NULL),
(277, NULL, NULL, NULL, NULL, NULL),
(278, NULL, NULL, NULL, NULL, NULL),
(279, NULL, NULL, NULL, NULL, NULL),
(280, NULL, NULL, NULL, NULL, NULL),
(281, NULL, NULL, NULL, NULL, NULL),
(282, NULL, NULL, NULL, NULL, NULL),
(283, NULL, NULL, NULL, NULL, NULL),
(284, NULL, NULL, NULL, NULL, NULL),
(285, NULL, NULL, NULL, NULL, NULL),
(286, NULL, NULL, NULL, NULL, NULL),
(287, NULL, NULL, NULL, NULL, NULL),
(288, NULL, NULL, NULL, NULL, NULL),
(289, NULL, NULL, NULL, NULL, NULL),
(290, NULL, NULL, NULL, NULL, NULL),
(291, NULL, NULL, NULL, NULL, NULL),
(292, NULL, NULL, NULL, NULL, NULL),
(293, NULL, NULL, NULL, NULL, NULL),
(294, NULL, NULL, NULL, NULL, NULL),
(295, NULL, NULL, NULL, NULL, NULL),
(296, NULL, NULL, NULL, NULL, NULL),
(297, NULL, NULL, NULL, NULL, NULL),
(298, NULL, NULL, NULL, NULL, NULL),
(299, NULL, NULL, NULL, NULL, NULL),
(300, NULL, NULL, NULL, NULL, NULL),
(301, NULL, NULL, NULL, NULL, NULL),
(302, NULL, NULL, NULL, NULL, NULL),
(303, NULL, NULL, NULL, NULL, NULL),
(304, NULL, NULL, NULL, NULL, NULL),
(305, NULL, NULL, NULL, NULL, NULL),
(306, NULL, NULL, NULL, NULL, NULL),
(307, NULL, NULL, NULL, NULL, NULL),
(308, NULL, NULL, NULL, NULL, NULL),
(309, NULL, NULL, NULL, NULL, NULL),
(310, NULL, NULL, NULL, NULL, NULL),
(311, NULL, NULL, NULL, NULL, NULL),
(312, NULL, NULL, NULL, NULL, NULL),
(313, NULL, NULL, NULL, NULL, NULL),
(314, NULL, NULL, NULL, NULL, NULL),
(315, NULL, NULL, NULL, NULL, NULL),
(316, NULL, NULL, NULL, NULL, NULL),
(317, NULL, NULL, NULL, NULL, NULL),
(318, NULL, NULL, NULL, NULL, NULL),
(319, NULL, NULL, NULL, NULL, NULL),
(320, NULL, NULL, NULL, NULL, NULL),
(321, NULL, NULL, NULL, NULL, NULL),
(322, NULL, NULL, NULL, NULL, NULL),
(323, NULL, NULL, NULL, NULL, NULL),
(324, NULL, NULL, NULL, NULL, NULL),
(325, NULL, NULL, NULL, NULL, NULL),
(326, NULL, NULL, NULL, NULL, NULL),
(327, NULL, NULL, NULL, NULL, NULL),
(328, NULL, NULL, NULL, NULL, NULL),
(329, NULL, NULL, NULL, NULL, NULL),
(330, NULL, NULL, NULL, NULL, NULL),
(331, NULL, NULL, NULL, NULL, NULL),
(332, NULL, NULL, NULL, NULL, NULL),
(333, NULL, NULL, NULL, NULL, NULL),
(334, NULL, NULL, NULL, NULL, NULL),
(335, NULL, NULL, NULL, NULL, NULL),
(336, NULL, NULL, NULL, NULL, NULL),
(337, NULL, NULL, NULL, NULL, NULL),
(338, NULL, NULL, NULL, NULL, NULL),
(339, NULL, NULL, NULL, NULL, NULL),
(340, NULL, NULL, NULL, NULL, NULL),
(341, NULL, NULL, NULL, NULL, NULL),
(342, NULL, NULL, NULL, NULL, NULL),
(343, NULL, NULL, NULL, NULL, NULL),
(344, NULL, NULL, NULL, NULL, NULL),
(345, NULL, NULL, NULL, NULL, NULL),
(346, NULL, NULL, NULL, NULL, NULL),
(347, NULL, NULL, NULL, NULL, NULL),
(348, NULL, NULL, NULL, NULL, NULL),
(349, NULL, NULL, NULL, NULL, NULL),
(350, NULL, NULL, NULL, NULL, NULL),
(351, NULL, NULL, NULL, NULL, NULL),
(352, NULL, NULL, NULL, NULL, NULL),
(353, NULL, NULL, NULL, NULL, NULL),
(354, NULL, NULL, NULL, NULL, NULL),
(355, NULL, NULL, NULL, NULL, NULL),
(356, NULL, NULL, NULL, NULL, NULL),
(357, NULL, NULL, NULL, NULL, NULL),
(358, NULL, NULL, NULL, NULL, NULL),
(359, NULL, NULL, NULL, NULL, NULL),
(360, NULL, NULL, NULL, NULL, NULL),
(361, NULL, NULL, NULL, NULL, NULL),
(362, NULL, NULL, NULL, NULL, NULL),
(363, NULL, NULL, NULL, NULL, NULL),
(364, NULL, NULL, NULL, NULL, NULL),
(365, NULL, NULL, NULL, NULL, NULL),
(366, NULL, NULL, NULL, NULL, NULL),
(367, NULL, NULL, NULL, NULL, NULL),
(368, NULL, NULL, NULL, NULL, NULL),
(369, NULL, NULL, NULL, NULL, NULL),
(370, NULL, NULL, NULL, NULL, NULL),
(371, NULL, NULL, NULL, NULL, NULL),
(372, NULL, NULL, NULL, NULL, NULL),
(373, NULL, NULL, NULL, NULL, NULL),
(374, NULL, NULL, NULL, NULL, NULL),
(375, NULL, NULL, NULL, NULL, NULL),
(376, NULL, NULL, NULL, NULL, NULL),
(377, NULL, NULL, NULL, NULL, NULL),
(378, NULL, NULL, NULL, NULL, NULL),
(379, NULL, NULL, NULL, NULL, NULL),
(380, NULL, NULL, NULL, NULL, NULL),
(381, NULL, NULL, NULL, NULL, NULL),
(382, NULL, NULL, NULL, NULL, NULL),
(383, NULL, NULL, NULL, NULL, NULL),
(384, NULL, NULL, NULL, NULL, NULL),
(385, NULL, NULL, NULL, NULL, NULL),
(386, NULL, NULL, NULL, NULL, NULL),
(387, NULL, NULL, NULL, NULL, NULL),
(388, NULL, NULL, NULL, NULL, NULL),
(389, NULL, NULL, NULL, NULL, NULL),
(390, NULL, NULL, NULL, NULL, NULL),
(391, NULL, NULL, NULL, NULL, NULL),
(392, NULL, NULL, NULL, NULL, NULL),
(393, NULL, NULL, NULL, NULL, NULL),
(394, NULL, NULL, NULL, NULL, NULL),
(395, NULL, NULL, NULL, NULL, NULL),
(396, NULL, NULL, NULL, NULL, NULL),
(397, NULL, NULL, NULL, NULL, NULL),
(398, NULL, NULL, NULL, NULL, NULL),
(399, NULL, NULL, NULL, NULL, NULL),
(400, NULL, NULL, NULL, NULL, NULL),
(401, NULL, NULL, NULL, NULL, NULL),
(402, NULL, NULL, NULL, NULL, NULL),
(403, NULL, NULL, NULL, NULL, NULL),
(404, NULL, NULL, NULL, NULL, NULL),
(405, NULL, NULL, NULL, NULL, NULL),
(406, NULL, NULL, NULL, NULL, NULL),
(407, NULL, NULL, NULL, NULL, NULL),
(408, NULL, NULL, NULL, NULL, NULL),
(409, NULL, NULL, NULL, NULL, NULL),
(410, NULL, NULL, NULL, NULL, NULL),
(411, NULL, NULL, NULL, NULL, NULL),
(412, NULL, NULL, NULL, NULL, NULL),
(413, NULL, NULL, NULL, NULL, NULL),
(414, NULL, NULL, NULL, NULL, NULL),
(415, NULL, NULL, NULL, NULL, NULL),
(416, NULL, NULL, NULL, NULL, NULL),
(417, NULL, NULL, NULL, NULL, NULL),
(418, NULL, NULL, NULL, NULL, NULL),
(419, NULL, NULL, NULL, NULL, NULL),
(420, NULL, NULL, NULL, NULL, NULL),
(421, NULL, NULL, NULL, NULL, NULL),
(422, NULL, NULL, NULL, NULL, NULL),
(423, NULL, NULL, NULL, NULL, NULL),
(424, NULL, NULL, NULL, NULL, NULL),
(425, NULL, NULL, NULL, NULL, NULL),
(426, NULL, NULL, NULL, NULL, NULL),
(427, NULL, NULL, NULL, NULL, NULL),
(428, NULL, NULL, NULL, NULL, NULL),
(429, NULL, NULL, NULL, NULL, NULL),
(430, NULL, NULL, NULL, NULL, NULL),
(431, NULL, NULL, NULL, NULL, NULL),
(432, NULL, NULL, NULL, NULL, NULL),
(433, NULL, NULL, NULL, NULL, NULL),
(434, NULL, NULL, NULL, NULL, NULL),
(435, NULL, NULL, NULL, NULL, NULL),
(436, NULL, NULL, NULL, NULL, NULL),
(437, NULL, NULL, NULL, NULL, NULL),
(438, NULL, NULL, NULL, NULL, NULL),
(439, NULL, NULL, NULL, NULL, NULL),
(440, NULL, NULL, NULL, NULL, NULL),
(441, NULL, NULL, NULL, NULL, NULL),
(442, NULL, NULL, NULL, NULL, NULL),
(443, NULL, NULL, NULL, NULL, NULL),
(444, NULL, NULL, NULL, NULL, NULL),
(445, NULL, NULL, NULL, NULL, NULL),
(446, NULL, NULL, NULL, NULL, NULL),
(447, NULL, NULL, NULL, NULL, NULL),
(448, NULL, NULL, NULL, NULL, NULL),
(449, NULL, NULL, NULL, NULL, NULL),
(450, NULL, NULL, NULL, NULL, NULL),
(451, NULL, NULL, NULL, NULL, NULL),
(452, NULL, NULL, NULL, NULL, NULL),
(453, NULL, NULL, NULL, NULL, NULL),
(454, NULL, NULL, NULL, NULL, NULL),
(455, NULL, NULL, NULL, NULL, NULL),
(456, NULL, NULL, NULL, NULL, NULL),
(457, NULL, NULL, NULL, NULL, NULL),
(458, NULL, NULL, NULL, NULL, NULL),
(459, NULL, NULL, NULL, NULL, NULL),
(460, NULL, NULL, NULL, NULL, NULL),
(461, NULL, NULL, NULL, NULL, NULL),
(462, NULL, NULL, NULL, NULL, NULL),
(463, NULL, NULL, NULL, NULL, NULL),
(464, NULL, NULL, NULL, NULL, NULL),
(465, NULL, NULL, NULL, NULL, NULL),
(466, NULL, NULL, NULL, NULL, NULL),
(467, NULL, NULL, NULL, NULL, NULL),
(468, NULL, NULL, NULL, NULL, NULL),
(469, NULL, NULL, NULL, NULL, NULL),
(470, NULL, NULL, NULL, NULL, NULL),
(471, NULL, NULL, NULL, NULL, NULL),
(472, NULL, NULL, NULL, NULL, NULL),
(473, NULL, NULL, NULL, NULL, NULL),
(474, NULL, NULL, NULL, NULL, NULL),
(475, NULL, NULL, NULL, NULL, NULL),
(476, NULL, NULL, NULL, NULL, NULL),
(477, NULL, NULL, NULL, NULL, NULL),
(478, NULL, NULL, NULL, NULL, NULL),
(479, NULL, NULL, NULL, NULL, NULL),
(480, NULL, NULL, NULL, NULL, NULL),
(481, NULL, NULL, NULL, NULL, NULL),
(482, NULL, NULL, NULL, NULL, NULL),
(483, NULL, NULL, NULL, NULL, NULL),
(484, NULL, NULL, NULL, NULL, NULL),
(485, NULL, NULL, NULL, NULL, NULL),
(486, NULL, NULL, NULL, NULL, NULL),
(487, NULL, NULL, NULL, NULL, NULL),
(488, NULL, NULL, NULL, NULL, NULL),
(489, NULL, NULL, NULL, NULL, NULL),
(490, NULL, NULL, NULL, NULL, NULL),
(491, NULL, NULL, NULL, NULL, NULL),
(492, NULL, NULL, NULL, NULL, NULL),
(493, NULL, NULL, NULL, NULL, NULL),
(494, NULL, NULL, NULL, NULL, NULL),
(495, NULL, NULL, NULL, NULL, NULL),
(496, NULL, NULL, NULL, NULL, NULL),
(497, NULL, NULL, NULL, NULL, NULL),
(498, NULL, NULL, NULL, NULL, NULL),
(499, NULL, NULL, NULL, NULL, NULL),
(500, NULL, NULL, NULL, NULL, NULL),
(501, NULL, NULL, NULL, NULL, NULL),
(502, NULL, NULL, NULL, NULL, NULL),
(503, NULL, NULL, NULL, NULL, NULL),
(504, NULL, NULL, NULL, NULL, NULL),
(505, NULL, NULL, NULL, NULL, NULL),
(506, NULL, NULL, NULL, NULL, NULL),
(507, NULL, NULL, NULL, NULL, NULL),
(508, NULL, NULL, NULL, NULL, NULL),
(509, NULL, NULL, NULL, NULL, NULL),
(510, NULL, NULL, NULL, NULL, NULL),
(511, NULL, NULL, NULL, NULL, NULL),
(512, NULL, NULL, NULL, NULL, NULL),
(513, NULL, NULL, NULL, NULL, NULL),
(514, NULL, NULL, NULL, NULL, NULL),
(515, NULL, NULL, NULL, NULL, NULL),
(516, NULL, NULL, NULL, NULL, NULL),
(517, NULL, NULL, NULL, NULL, NULL),
(518, NULL, NULL, NULL, NULL, NULL),
(519, NULL, NULL, NULL, NULL, NULL),
(520, NULL, NULL, NULL, NULL, NULL),
(521, NULL, NULL, NULL, NULL, NULL),
(522, NULL, NULL, NULL, NULL, NULL),
(523, NULL, NULL, NULL, NULL, NULL),
(524, NULL, NULL, NULL, NULL, NULL),
(525, NULL, NULL, NULL, NULL, NULL),
(526, NULL, NULL, NULL, NULL, NULL),
(527, NULL, NULL, NULL, NULL, NULL),
(528, NULL, NULL, NULL, NULL, NULL),
(529, NULL, NULL, NULL, NULL, NULL),
(530, NULL, NULL, NULL, NULL, NULL),
(531, NULL, NULL, NULL, NULL, NULL),
(532, NULL, NULL, NULL, NULL, NULL),
(533, NULL, NULL, NULL, NULL, NULL),
(534, NULL, NULL, NULL, NULL, NULL),
(535, NULL, NULL, NULL, NULL, NULL),
(536, NULL, NULL, NULL, NULL, NULL),
(537, NULL, NULL, NULL, NULL, NULL),
(538, NULL, NULL, NULL, NULL, NULL),
(539, NULL, NULL, NULL, NULL, NULL),
(540, NULL, NULL, NULL, NULL, NULL),
(541, NULL, NULL, NULL, NULL, NULL),
(542, NULL, NULL, NULL, NULL, NULL),
(543, NULL, NULL, NULL, NULL, NULL),
(544, NULL, NULL, NULL, NULL, NULL),
(545, NULL, NULL, NULL, NULL, NULL),
(546, NULL, NULL, NULL, NULL, NULL),
(547, NULL, NULL, NULL, NULL, NULL),
(548, NULL, NULL, NULL, NULL, NULL),
(549, NULL, NULL, NULL, NULL, NULL),
(550, NULL, NULL, NULL, NULL, NULL),
(551, NULL, NULL, NULL, NULL, NULL),
(552, NULL, NULL, NULL, NULL, NULL),
(553, NULL, NULL, NULL, NULL, NULL),
(554, NULL, NULL, NULL, NULL, NULL),
(555, NULL, NULL, NULL, NULL, NULL),
(556, NULL, NULL, NULL, NULL, NULL),
(557, NULL, NULL, NULL, NULL, NULL),
(558, NULL, NULL, NULL, NULL, NULL),
(559, NULL, NULL, NULL, NULL, NULL),
(560, NULL, NULL, NULL, NULL, NULL),
(561, NULL, NULL, NULL, NULL, NULL),
(562, NULL, NULL, NULL, NULL, NULL),
(563, NULL, NULL, NULL, NULL, NULL),
(564, NULL, NULL, NULL, NULL, NULL),
(565, NULL, NULL, NULL, NULL, NULL),
(566, NULL, NULL, NULL, NULL, NULL),
(567, NULL, NULL, NULL, NULL, NULL),
(568, NULL, NULL, NULL, NULL, NULL),
(569, NULL, NULL, NULL, NULL, NULL),
(570, NULL, NULL, NULL, NULL, NULL),
(571, NULL, NULL, NULL, NULL, NULL),
(572, NULL, NULL, NULL, NULL, NULL),
(573, NULL, NULL, NULL, NULL, NULL),
(574, NULL, NULL, NULL, NULL, NULL),
(575, NULL, NULL, NULL, NULL, NULL),
(576, NULL, NULL, NULL, NULL, NULL),
(577, NULL, NULL, NULL, NULL, NULL),
(578, NULL, NULL, NULL, NULL, NULL),
(579, NULL, NULL, NULL, NULL, NULL),
(580, NULL, NULL, NULL, NULL, NULL),
(581, NULL, NULL, NULL, NULL, NULL),
(582, NULL, NULL, NULL, NULL, NULL),
(583, NULL, NULL, NULL, NULL, NULL),
(584, NULL, NULL, NULL, NULL, NULL),
(585, NULL, NULL, NULL, NULL, NULL),
(586, NULL, NULL, NULL, NULL, NULL),
(587, NULL, NULL, NULL, NULL, NULL),
(588, NULL, NULL, NULL, NULL, NULL),
(589, NULL, NULL, NULL, NULL, NULL),
(590, NULL, NULL, NULL, NULL, NULL),
(591, NULL, NULL, NULL, NULL, NULL),
(592, NULL, NULL, NULL, NULL, NULL),
(593, NULL, NULL, NULL, NULL, NULL),
(594, NULL, NULL, NULL, NULL, NULL),
(595, NULL, NULL, NULL, NULL, NULL),
(596, NULL, NULL, NULL, NULL, NULL),
(597, NULL, NULL, NULL, NULL, NULL),
(598, NULL, NULL, NULL, NULL, NULL),
(599, NULL, NULL, NULL, NULL, NULL),
(600, NULL, NULL, NULL, NULL, NULL),
(601, NULL, NULL, NULL, NULL, NULL),
(602, NULL, NULL, NULL, NULL, NULL),
(603, NULL, NULL, NULL, NULL, NULL),
(604, NULL, NULL, NULL, NULL, NULL),
(605, NULL, NULL, NULL, NULL, NULL),
(606, NULL, NULL, NULL, NULL, NULL),
(607, NULL, NULL, NULL, NULL, NULL),
(608, NULL, NULL, NULL, NULL, NULL),
(609, NULL, NULL, NULL, NULL, NULL),
(610, NULL, NULL, NULL, NULL, NULL),
(611, NULL, NULL, NULL, NULL, NULL),
(612, NULL, NULL, NULL, NULL, NULL),
(613, NULL, NULL, NULL, NULL, NULL),
(614, NULL, NULL, NULL, NULL, NULL),
(615, NULL, NULL, NULL, NULL, NULL),
(616, NULL, NULL, NULL, NULL, NULL),
(617, NULL, NULL, NULL, NULL, NULL),
(618, NULL, NULL, NULL, NULL, NULL),
(619, NULL, NULL, NULL, NULL, NULL),
(620, NULL, NULL, NULL, NULL, NULL),
(621, NULL, NULL, NULL, NULL, NULL),
(622, NULL, NULL, NULL, NULL, NULL),
(623, NULL, NULL, NULL, NULL, NULL),
(624, NULL, NULL, NULL, NULL, NULL),
(625, NULL, NULL, NULL, NULL, NULL),
(626, NULL, NULL, NULL, NULL, NULL),
(627, NULL, NULL, NULL, NULL, NULL),
(628, NULL, NULL, NULL, NULL, NULL),
(629, NULL, NULL, NULL, NULL, NULL),
(630, NULL, NULL, NULL, NULL, NULL),
(631, NULL, NULL, NULL, NULL, NULL),
(632, NULL, NULL, NULL, NULL, NULL),
(633, NULL, NULL, NULL, NULL, NULL),
(634, NULL, NULL, NULL, NULL, NULL),
(635, NULL, NULL, NULL, NULL, NULL),
(636, NULL, NULL, NULL, NULL, NULL),
(637, NULL, NULL, NULL, NULL, NULL),
(638, NULL, NULL, NULL, NULL, NULL),
(639, NULL, NULL, NULL, NULL, NULL),
(640, NULL, NULL, NULL, NULL, NULL),
(641, NULL, NULL, NULL, NULL, NULL),
(642, NULL, NULL, NULL, NULL, NULL),
(643, NULL, NULL, NULL, NULL, NULL),
(644, NULL, NULL, NULL, NULL, NULL),
(645, NULL, NULL, NULL, NULL, NULL),
(646, NULL, NULL, NULL, NULL, NULL),
(647, NULL, NULL, NULL, NULL, NULL),
(648, NULL, NULL, NULL, NULL, NULL),
(649, NULL, NULL, NULL, NULL, NULL),
(650, NULL, NULL, NULL, NULL, NULL),
(651, NULL, NULL, NULL, NULL, NULL),
(652, NULL, NULL, NULL, NULL, NULL),
(653, NULL, NULL, NULL, NULL, NULL),
(654, NULL, NULL, NULL, NULL, NULL),
(655, NULL, NULL, NULL, NULL, NULL),
(656, NULL, NULL, NULL, NULL, NULL),
(657, NULL, NULL, NULL, NULL, NULL),
(658, NULL, NULL, NULL, NULL, NULL),
(659, NULL, NULL, NULL, NULL, NULL),
(660, NULL, NULL, NULL, NULL, NULL),
(661, NULL, NULL, NULL, NULL, NULL),
(662, NULL, NULL, NULL, NULL, NULL),
(663, NULL, NULL, NULL, NULL, NULL),
(664, NULL, NULL, NULL, NULL, NULL),
(665, NULL, NULL, NULL, NULL, NULL),
(666, NULL, NULL, NULL, NULL, NULL),
(667, NULL, NULL, NULL, NULL, NULL),
(668, NULL, NULL, NULL, NULL, NULL),
(669, NULL, NULL, NULL, NULL, NULL),
(670, NULL, NULL, NULL, NULL, NULL),
(671, NULL, NULL, NULL, NULL, NULL),
(672, NULL, NULL, NULL, NULL, NULL),
(673, NULL, NULL, NULL, NULL, NULL),
(674, NULL, NULL, NULL, NULL, NULL),
(675, NULL, NULL, NULL, NULL, NULL),
(676, NULL, NULL, NULL, NULL, NULL),
(677, NULL, NULL, NULL, NULL, NULL),
(678, NULL, NULL, NULL, NULL, NULL),
(679, NULL, NULL, NULL, NULL, NULL),
(680, NULL, NULL, NULL, NULL, NULL),
(681, NULL, NULL, NULL, NULL, NULL),
(682, NULL, NULL, NULL, NULL, NULL),
(683, NULL, NULL, NULL, NULL, NULL),
(684, NULL, NULL, NULL, NULL, NULL),
(685, NULL, NULL, NULL, NULL, NULL),
(686, NULL, NULL, NULL, NULL, NULL),
(687, NULL, NULL, NULL, NULL, NULL),
(688, NULL, NULL, NULL, NULL, NULL),
(689, NULL, NULL, NULL, NULL, NULL),
(690, NULL, NULL, NULL, NULL, NULL),
(691, NULL, NULL, NULL, NULL, NULL),
(692, NULL, NULL, NULL, NULL, NULL),
(693, NULL, NULL, NULL, NULL, NULL),
(694, NULL, NULL, NULL, NULL, NULL),
(695, NULL, NULL, NULL, NULL, NULL),
(696, NULL, NULL, NULL, NULL, NULL),
(697, NULL, NULL, NULL, NULL, NULL),
(698, NULL, NULL, NULL, NULL, NULL),
(699, NULL, NULL, NULL, NULL, NULL),
(700, NULL, NULL, NULL, NULL, NULL),
(701, NULL, NULL, NULL, NULL, NULL),
(702, NULL, NULL, NULL, NULL, NULL),
(703, NULL, NULL, NULL, NULL, NULL),
(704, NULL, NULL, NULL, NULL, NULL),
(705, NULL, NULL, NULL, NULL, NULL),
(706, NULL, NULL, NULL, NULL, NULL),
(707, NULL, NULL, NULL, NULL, NULL),
(708, NULL, NULL, NULL, NULL, NULL),
(709, NULL, NULL, NULL, NULL, NULL),
(710, NULL, NULL, NULL, NULL, NULL),
(711, NULL, NULL, NULL, NULL, NULL),
(712, NULL, NULL, NULL, NULL, NULL),
(713, NULL, NULL, NULL, NULL, NULL),
(714, NULL, NULL, NULL, NULL, NULL),
(715, NULL, NULL, NULL, NULL, NULL),
(716, NULL, NULL, NULL, NULL, NULL),
(717, NULL, NULL, NULL, NULL, NULL),
(718, NULL, NULL, NULL, NULL, NULL),
(719, NULL, NULL, NULL, NULL, NULL),
(720, NULL, NULL, NULL, NULL, NULL),
(721, NULL, NULL, NULL, NULL, NULL),
(722, NULL, NULL, NULL, NULL, NULL),
(723, NULL, NULL, NULL, NULL, NULL),
(724, NULL, NULL, NULL, NULL, NULL),
(725, NULL, NULL, NULL, NULL, NULL),
(726, NULL, NULL, NULL, NULL, NULL),
(727, NULL, NULL, NULL, NULL, NULL),
(728, NULL, NULL, NULL, NULL, NULL),
(729, NULL, NULL, NULL, NULL, NULL),
(730, NULL, NULL, NULL, NULL, NULL),
(731, NULL, NULL, NULL, NULL, NULL),
(732, NULL, NULL, NULL, NULL, NULL),
(733, NULL, NULL, NULL, NULL, NULL),
(734, NULL, NULL, NULL, NULL, NULL),
(735, NULL, NULL, NULL, NULL, NULL),
(736, NULL, NULL, NULL, NULL, NULL),
(737, NULL, NULL, NULL, NULL, NULL),
(738, NULL, NULL, NULL, NULL, NULL),
(739, NULL, NULL, NULL, NULL, NULL),
(740, NULL, NULL, NULL, NULL, NULL),
(741, NULL, NULL, NULL, NULL, NULL),
(742, NULL, NULL, NULL, NULL, NULL),
(743, NULL, NULL, NULL, NULL, NULL),
(744, NULL, NULL, NULL, NULL, NULL),
(745, NULL, NULL, NULL, NULL, NULL),
(746, NULL, NULL, NULL, NULL, NULL),
(747, NULL, NULL, NULL, NULL, NULL),
(748, NULL, NULL, NULL, NULL, NULL),
(749, NULL, NULL, NULL, NULL, NULL),
(750, NULL, NULL, NULL, NULL, NULL),
(751, NULL, NULL, NULL, NULL, NULL),
(752, NULL, NULL, NULL, NULL, NULL),
(753, NULL, NULL, NULL, NULL, NULL),
(754, NULL, NULL, NULL, NULL, NULL),
(755, NULL, NULL, NULL, NULL, NULL),
(756, NULL, NULL, NULL, NULL, NULL),
(757, NULL, NULL, NULL, NULL, NULL),
(758, NULL, NULL, NULL, NULL, NULL),
(759, NULL, NULL, NULL, NULL, NULL),
(760, NULL, NULL, NULL, NULL, NULL),
(761, NULL, NULL, NULL, NULL, NULL),
(762, NULL, NULL, NULL, NULL, NULL),
(763, NULL, NULL, NULL, NULL, NULL),
(764, NULL, NULL, NULL, NULL, NULL),
(765, NULL, NULL, NULL, NULL, NULL),
(766, NULL, NULL, NULL, NULL, NULL),
(767, NULL, NULL, NULL, NULL, NULL),
(768, NULL, NULL, NULL, NULL, NULL),
(769, NULL, NULL, NULL, NULL, NULL),
(770, NULL, NULL, NULL, NULL, NULL),
(771, NULL, NULL, NULL, NULL, NULL),
(772, NULL, NULL, NULL, NULL, NULL),
(773, NULL, NULL, NULL, NULL, NULL),
(774, NULL, NULL, NULL, NULL, NULL),
(775, NULL, NULL, NULL, NULL, NULL),
(776, NULL, NULL, NULL, NULL, NULL),
(777, NULL, NULL, NULL, NULL, NULL),
(778, NULL, NULL, NULL, NULL, NULL),
(779, NULL, NULL, NULL, NULL, NULL),
(780, NULL, NULL, NULL, NULL, NULL),
(781, NULL, NULL, NULL, NULL, NULL),
(782, NULL, NULL, NULL, NULL, NULL),
(783, NULL, NULL, NULL, NULL, NULL),
(784, NULL, NULL, NULL, NULL, NULL),
(785, NULL, NULL, NULL, NULL, NULL),
(786, NULL, NULL, NULL, NULL, NULL),
(787, NULL, NULL, NULL, NULL, NULL),
(788, NULL, NULL, NULL, NULL, NULL),
(789, NULL, NULL, NULL, NULL, NULL),
(790, NULL, NULL, NULL, NULL, NULL),
(791, NULL, NULL, NULL, NULL, NULL),
(792, NULL, NULL, NULL, NULL, NULL),
(793, NULL, NULL, NULL, NULL, NULL),
(794, NULL, NULL, NULL, NULL, NULL),
(795, NULL, NULL, NULL, NULL, NULL),
(796, NULL, NULL, NULL, NULL, NULL),
(797, NULL, NULL, NULL, NULL, NULL),
(798, NULL, NULL, NULL, NULL, NULL),
(799, NULL, NULL, NULL, NULL, NULL),
(800, NULL, NULL, NULL, NULL, NULL),
(801, NULL, NULL, NULL, NULL, NULL),
(802, NULL, NULL, NULL, NULL, NULL),
(803, NULL, NULL, NULL, NULL, NULL),
(804, NULL, NULL, NULL, NULL, NULL),
(805, NULL, NULL, NULL, NULL, NULL),
(806, NULL, NULL, NULL, NULL, NULL),
(807, NULL, NULL, NULL, NULL, NULL),
(808, NULL, NULL, NULL, NULL, NULL),
(809, NULL, NULL, NULL, NULL, NULL),
(810, NULL, NULL, NULL, NULL, NULL),
(811, NULL, NULL, NULL, NULL, NULL),
(812, NULL, NULL, NULL, NULL, NULL),
(813, NULL, NULL, NULL, NULL, NULL),
(814, NULL, NULL, NULL, NULL, NULL),
(815, NULL, NULL, NULL, NULL, NULL),
(816, NULL, NULL, NULL, NULL, NULL),
(817, NULL, NULL, NULL, NULL, NULL),
(818, NULL, NULL, NULL, NULL, NULL),
(819, NULL, NULL, NULL, NULL, NULL),
(820, NULL, NULL, NULL, NULL, NULL),
(821, NULL, NULL, NULL, NULL, NULL),
(822, NULL, NULL, NULL, NULL, NULL),
(823, NULL, NULL, NULL, NULL, NULL),
(824, NULL, NULL, NULL, NULL, NULL),
(825, NULL, NULL, NULL, NULL, NULL),
(826, NULL, NULL, NULL, NULL, NULL),
(827, NULL, NULL, NULL, NULL, NULL),
(828, NULL, NULL, NULL, NULL, NULL),
(829, NULL, NULL, NULL, NULL, NULL),
(830, NULL, NULL, NULL, NULL, NULL),
(831, NULL, NULL, NULL, NULL, NULL),
(832, NULL, NULL, NULL, NULL, NULL),
(833, NULL, NULL, NULL, NULL, NULL),
(834, NULL, NULL, NULL, NULL, NULL),
(835, NULL, NULL, NULL, NULL, NULL),
(836, NULL, NULL, NULL, NULL, NULL),
(837, NULL, NULL, NULL, NULL, NULL),
(838, NULL, NULL, NULL, NULL, NULL),
(839, NULL, NULL, NULL, NULL, NULL),
(840, NULL, NULL, NULL, NULL, NULL),
(841, NULL, NULL, NULL, NULL, NULL),
(842, NULL, NULL, NULL, NULL, NULL),
(843, NULL, NULL, NULL, NULL, NULL),
(844, NULL, NULL, NULL, NULL, NULL),
(845, NULL, NULL, NULL, NULL, NULL),
(846, NULL, NULL, NULL, NULL, NULL),
(847, NULL, NULL, NULL, NULL, NULL),
(848, NULL, NULL, NULL, NULL, NULL),
(849, NULL, NULL, NULL, NULL, NULL),
(850, NULL, NULL, NULL, NULL, NULL),
(851, NULL, NULL, NULL, NULL, NULL),
(852, NULL, NULL, NULL, NULL, NULL),
(853, NULL, NULL, NULL, NULL, NULL),
(854, NULL, NULL, NULL, NULL, NULL),
(855, NULL, NULL, NULL, NULL, NULL),
(856, NULL, NULL, NULL, NULL, NULL),
(857, NULL, NULL, NULL, NULL, NULL),
(858, NULL, NULL, NULL, NULL, NULL),
(859, NULL, NULL, NULL, NULL, NULL),
(860, NULL, NULL, NULL, NULL, NULL),
(861, NULL, NULL, NULL, NULL, NULL),
(862, NULL, NULL, NULL, NULL, NULL),
(863, NULL, NULL, NULL, NULL, NULL),
(864, NULL, NULL, NULL, NULL, NULL),
(865, NULL, NULL, NULL, NULL, NULL),
(866, NULL, NULL, NULL, NULL, NULL),
(867, NULL, NULL, NULL, NULL, NULL),
(868, NULL, NULL, NULL, NULL, NULL),
(869, NULL, NULL, NULL, NULL, NULL),
(870, NULL, NULL, NULL, NULL, NULL),
(871, NULL, NULL, NULL, NULL, NULL),
(872, NULL, NULL, NULL, NULL, NULL),
(873, NULL, NULL, NULL, NULL, NULL),
(874, NULL, NULL, NULL, NULL, NULL),
(875, NULL, NULL, NULL, NULL, NULL),
(876, NULL, NULL, NULL, NULL, NULL),
(877, NULL, NULL, NULL, NULL, NULL),
(878, NULL, NULL, NULL, NULL, NULL),
(879, NULL, NULL, NULL, NULL, NULL),
(880, NULL, NULL, NULL, NULL, NULL),
(881, NULL, NULL, NULL, NULL, NULL),
(882, NULL, NULL, NULL, NULL, NULL),
(883, NULL, NULL, NULL, NULL, NULL),
(884, NULL, NULL, NULL, NULL, NULL),
(885, NULL, NULL, NULL, NULL, NULL),
(886, NULL, NULL, NULL, NULL, NULL),
(887, NULL, NULL, NULL, NULL, NULL),
(888, NULL, NULL, NULL, NULL, NULL),
(889, NULL, NULL, NULL, NULL, NULL),
(890, NULL, NULL, NULL, NULL, NULL),
(891, NULL, NULL, NULL, NULL, NULL),
(892, NULL, NULL, NULL, NULL, NULL),
(893, NULL, NULL, NULL, NULL, NULL),
(894, NULL, NULL, NULL, NULL, NULL),
(895, NULL, NULL, NULL, NULL, NULL),
(896, NULL, NULL, NULL, NULL, NULL),
(897, NULL, NULL, NULL, NULL, NULL),
(898, NULL, NULL, NULL, NULL, NULL),
(899, NULL, NULL, NULL, NULL, NULL),
(900, NULL, NULL, NULL, NULL, NULL),
(901, NULL, NULL, NULL, NULL, NULL),
(902, NULL, NULL, NULL, NULL, NULL),
(903, NULL, NULL, NULL, NULL, NULL),
(904, NULL, NULL, NULL, NULL, NULL),
(905, NULL, NULL, NULL, NULL, NULL),
(906, NULL, NULL, NULL, NULL, NULL),
(907, NULL, NULL, NULL, NULL, NULL),
(908, NULL, NULL, NULL, NULL, NULL),
(909, NULL, NULL, NULL, NULL, NULL),
(910, NULL, NULL, NULL, NULL, NULL),
(911, NULL, NULL, NULL, NULL, NULL),
(912, NULL, NULL, NULL, NULL, NULL),
(913, NULL, NULL, NULL, NULL, NULL),
(914, NULL, NULL, NULL, NULL, NULL),
(915, NULL, NULL, NULL, NULL, NULL),
(916, NULL, NULL, NULL, NULL, NULL),
(917, NULL, NULL, NULL, NULL, NULL),
(918, NULL, NULL, NULL, NULL, NULL),
(919, NULL, NULL, NULL, NULL, NULL),
(920, NULL, NULL, NULL, NULL, NULL),
(921, NULL, NULL, NULL, NULL, NULL),
(922, NULL, NULL, NULL, NULL, NULL),
(923, NULL, NULL, NULL, NULL, NULL),
(924, NULL, NULL, NULL, NULL, NULL),
(925, NULL, NULL, NULL, NULL, NULL),
(926, NULL, NULL, NULL, NULL, NULL),
(927, NULL, NULL, NULL, NULL, NULL),
(928, NULL, NULL, NULL, NULL, NULL),
(929, NULL, NULL, NULL, NULL, NULL),
(930, NULL, NULL, NULL, NULL, NULL),
(931, NULL, NULL, NULL, NULL, NULL),
(932, NULL, NULL, NULL, NULL, NULL),
(933, NULL, NULL, NULL, NULL, NULL),
(934, NULL, NULL, NULL, NULL, NULL),
(935, NULL, NULL, NULL, NULL, NULL),
(936, NULL, NULL, NULL, NULL, NULL),
(937, NULL, NULL, NULL, NULL, NULL),
(938, NULL, NULL, NULL, NULL, NULL),
(939, NULL, NULL, NULL, NULL, NULL),
(940, NULL, NULL, NULL, NULL, NULL),
(941, NULL, NULL, NULL, NULL, NULL),
(942, NULL, NULL, NULL, NULL, NULL),
(943, NULL, NULL, NULL, NULL, NULL),
(944, NULL, NULL, NULL, NULL, NULL),
(945, NULL, NULL, NULL, NULL, NULL),
(946, NULL, NULL, NULL, NULL, NULL),
(947, NULL, NULL, NULL, NULL, NULL),
(948, NULL, NULL, NULL, NULL, NULL),
(949, NULL, NULL, NULL, NULL, NULL),
(950, NULL, NULL, NULL, NULL, NULL),
(951, NULL, NULL, NULL, NULL, NULL),
(952, NULL, NULL, NULL, NULL, NULL),
(953, NULL, NULL, NULL, NULL, NULL),
(954, NULL, NULL, NULL, NULL, NULL),
(955, NULL, NULL, NULL, NULL, NULL),
(956, NULL, NULL, NULL, NULL, NULL),
(957, NULL, NULL, NULL, NULL, NULL),
(958, NULL, NULL, NULL, NULL, NULL),
(959, NULL, NULL, NULL, NULL, NULL),
(960, NULL, NULL, NULL, NULL, NULL),
(961, NULL, NULL, NULL, NULL, NULL),
(962, NULL, NULL, NULL, NULL, NULL),
(963, NULL, NULL, NULL, NULL, NULL),
(964, NULL, NULL, NULL, NULL, NULL),
(965, NULL, NULL, NULL, NULL, NULL),
(966, NULL, NULL, NULL, NULL, NULL),
(967, NULL, NULL, NULL, NULL, NULL),
(968, NULL, NULL, NULL, NULL, NULL),
(969, NULL, NULL, NULL, NULL, NULL),
(970, NULL, NULL, NULL, NULL, NULL),
(971, NULL, NULL, NULL, NULL, NULL),
(972, NULL, NULL, NULL, NULL, NULL),
(973, NULL, NULL, NULL, NULL, NULL),
(974, NULL, NULL, NULL, NULL, NULL),
(975, NULL, NULL, NULL, NULL, NULL),
(976, NULL, NULL, NULL, NULL, NULL),
(977, NULL, NULL, NULL, NULL, NULL),
(978, NULL, NULL, NULL, NULL, NULL),
(979, NULL, NULL, NULL, NULL, NULL),
(980, NULL, NULL, NULL, NULL, NULL),
(981, NULL, NULL, NULL, NULL, NULL),
(982, NULL, NULL, NULL, NULL, NULL),
(983, NULL, NULL, NULL, NULL, NULL),
(984, NULL, NULL, NULL, NULL, NULL),
(985, NULL, NULL, NULL, NULL, NULL),
(986, NULL, NULL, NULL, NULL, NULL),
(987, NULL, NULL, NULL, NULL, NULL),
(988, NULL, NULL, NULL, NULL, NULL),
(989, NULL, NULL, NULL, NULL, NULL),
(990, NULL, NULL, NULL, NULL, NULL),
(991, NULL, NULL, NULL, NULL, NULL),
(992, NULL, NULL, NULL, NULL, NULL),
(993, NULL, NULL, NULL, NULL, NULL),
(994, NULL, NULL, NULL, NULL, NULL),
(995, NULL, NULL, NULL, NULL, NULL),
(996, NULL, NULL, NULL, NULL, NULL),
(997, NULL, NULL, NULL, NULL, NULL),
(998, NULL, NULL, NULL, NULL, NULL),
(999, NULL, NULL, NULL, NULL, NULL);

--
-- Indeksy dla zrzutów tabel
--

--
-- Indeksy dla tabeli `atrakcja`
--
ALTER TABLE `atrakcja`
  ADD PRIMARY KEY (`id_atrakcji`),
  ADD KEY `atrakcja_zegarek_FK` (`zegarek_id_zegarka`),
  ADD KEY `atrakcja_cennik_FK` (`cennik_id_cennika`),
  ADD KEY `atrakcja_nawa_atrakcji_FK` (`lista_atrakcji_nazwa`);

--
-- Indeksy dla tabeli `bilet`
--
ALTER TABLE `bilet`
  ADD PRIMARY KEY (`id_biletu`),
  ADD KEY `bilet_klient_FK` (`klient_id_klienta`),
  ADD KEY `bilet_platnosc_FK` (`platnosc_id_platnosci`),
  ADD KEY `bilet_cennik_FK` (`cennik_id_cennika`);

--
-- Indeksy dla tabeli `cennik`
--
ALTER TABLE `cennik`
  ADD PRIMARY KEY (`id_cennika`);

--
-- Indeksy dla tabeli `doplata`
--
ALTER TABLE `doplata`
  ADD PRIMARY KEY (`id_doplaty`);

--
-- Indeksy dla tabeli `karnet`
--
ALTER TABLE `karnet`
  ADD PRIMARY KEY (`id_karnetu`),
  ADD KEY `karnet_klient_FK` (`klient_id_klienta`),
  ADD KEY `karnet_platnosc_FK` (`platnosc_id_platnosci`),
  ADD KEY `karnet_cennik_FK` (`cennik_id_cennika`);

--
-- Indeksy dla tabeli `klient`
--
ALTER TABLE `klient`
  ADD PRIMARY KEY (`id_klienta`);

--
-- Indeksy dla tabeli `konta_w_systemie`
--
ALTER TABLE `konta_w_systemie`
  ADD PRIMARY KEY (`login`);

--
-- Indeksy dla tabeli `lista_atrakcji`
--
ALTER TABLE `lista_atrakcji`
  ADD PRIMARY KEY (`nazwa`);

--
-- Indeksy dla tabeli `platnosc`
--
ALTER TABLE `platnosc`
  ADD PRIMARY KEY (`id_platnosci`),
  ADD KEY `platnosc_klient_FK` (`klient_id_klienta`);

--
-- Indeksy dla tabeli `zegarek`
--
ALTER TABLE `zegarek`
  ADD PRIMARY KEY (`id_zegarka`),
  ADD UNIQUE KEY `karnet_id_karnetu` (`karnet_id_karnetu`),
  ADD KEY `zegarek_klient_FK` (`klient_id_klienta`),
  ADD KEY `zegarek_bilet_FK` (`bilet_id_biletu`),
  ADD KEY `zegarek_karnet_FK` (`karnet_id_karnetu`);

--
-- AUTO_INCREMENT dla zrzuconych tabel
--

--
-- AUTO_INCREMENT dla tabeli `atrakcja`
--
ALTER TABLE `atrakcja`
  MODIFY `id_atrakcji` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT dla tabeli `bilet`
--
ALTER TABLE `bilet`
  MODIFY `id_biletu` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT dla tabeli `cennik`
--
ALTER TABLE `cennik`
  MODIFY `id_cennika` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;

--
-- AUTO_INCREMENT dla tabeli `doplata`
--
ALTER TABLE `doplata`
  MODIFY `id_doplaty` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT dla tabeli `karnet`
--
ALTER TABLE `karnet`
  MODIFY `id_karnetu` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT dla tabeli `klient`
--
ALTER TABLE `klient`
  MODIFY `id_klienta` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT dla tabeli `platnosc`
--
ALTER TABLE `platnosc`
  MODIFY `id_platnosci` int(11) NOT NULL AUTO_INCREMENT;

--
-- Ograniczenia dla zrzutów tabel
--

--
-- Ograniczenia dla tabeli `atrakcja`
--
ALTER TABLE `atrakcja`
  ADD CONSTRAINT `atrakcja_cennik_FK` FOREIGN KEY (`cennik_id_cennika`) REFERENCES `cennik` (`id_cennika`),
  ADD CONSTRAINT `atrakcja_nawa_atrakcji_FK` FOREIGN KEY (`lista_atrakcji_nazwa`) REFERENCES `lista_atrakcji` (`nazwa`),
  ADD CONSTRAINT `atrakcja_zegarek_FK` FOREIGN KEY (`zegarek_id_zegarka`) REFERENCES `zegarek` (`id_zegarka`);

--
-- Ograniczenia dla tabeli `bilet`
--
ALTER TABLE `bilet`
  ADD CONSTRAINT `bilet_cennik_FK` FOREIGN KEY (`cennik_id_cennika`) REFERENCES `cennik` (`id_cennika`),
  ADD CONSTRAINT `bilet_klient_FK` FOREIGN KEY (`klient_id_klienta`) REFERENCES `klient` (`id_klienta`),
  ADD CONSTRAINT `bilet_platnosc_FK` FOREIGN KEY (`platnosc_id_platnosci`) REFERENCES `platnosc` (`id_platnosci`);

--
-- Ograniczenia dla tabeli `karnet`
--
ALTER TABLE `karnet`
  ADD CONSTRAINT `karnet_cennik_FK` FOREIGN KEY (`cennik_id_cennika`) REFERENCES `cennik` (`id_cennika`),
  ADD CONSTRAINT `karnet_klient_FK` FOREIGN KEY (`klient_id_klienta`) REFERENCES `klient` (`id_klienta`),
  ADD CONSTRAINT `karnet_platnosc_FK` FOREIGN KEY (`platnosc_id_platnosci`) REFERENCES `platnosc` (`id_platnosci`);

--
-- Ograniczenia dla tabeli `platnosc`
--
ALTER TABLE `platnosc`
  ADD CONSTRAINT `platnosc_klient_FK` FOREIGN KEY (`klient_id_klienta`) REFERENCES `klient` (`id_klienta`);

--
-- Ograniczenia dla tabeli `zegarek`
--
ALTER TABLE `zegarek`
  ADD CONSTRAINT `zegarek_bilet_FK` FOREIGN KEY (`bilet_id_biletu`) REFERENCES `bilet` (`id_biletu`),
  ADD CONSTRAINT `zegarek_karnet_FK` FOREIGN KEY (`karnet_id_karnetu`) REFERENCES `karnet` (`id_karnetu`),
  ADD CONSTRAINT `zegarek_klient_FK` FOREIGN KEY (`klient_id_klienta`) REFERENCES `klient` (`id_klienta`);
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
